...
def costum_label_callback(query: CallbackQuery, query_data, update: Update, context: CallbackContext):
    costum_label = query_data['text']
    movie_id = query_data['data']
    add_label_to_db(movie_id,costum_label,chatbot().db)
    ...
    context.bot.send_message(chat_id=update.effective_chat.id,text='You costum label is submitted! \nDo you want to write comment for is movie?',reply_markup=reply_markup)
    undo_callback_chat_all(update.effective_chat, chatbot().db)
chatbot = GetChatbot()
Callback = chatbot, costum_label_callback